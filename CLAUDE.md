...
所有内容产出必须通过三环校准，任何一环不通过都应回头修正：

| 环 | 阶段 | 校准问题 |
|----|------|---------|
| **A-Gate**（观众需求） | 选题 | 这个选题是「观众想看」还是「我想讲」？ |
| **C-Gate**（内容结构） | 大纲 | 结构是否匹配平台算法（完播率/互动/搜索）？ |
| **R-Gate**（共鸣效果） | 终审 | 新手能听懂吗？信任感够吗？节奏拖吗？ |

**五大自嗨类型**需警惕：参数自嗨、技术自嗨、细节自嗨、资讯自嗨、情怀自嗨。

### D1-D6 产品研究框架

产品资料收集遵循六层递进结构，各层可并行启动：

| 层 | 名称 | 核心任务 |
|----|------|---------|
| D1 | 产品事实层 | 规格、定价、配置——参数要翻译成体验语言 |
| D2 | 体感层（品牌认知层） | 实机手感、材质、设计语言——需实机到手后补 |
| D3 | 观众认知层 | 评论区调研，提炼「观众最关心的 3 个问题」作为脚本骨架 |
| D4 | 叙事张力层 | 按决策逻辑排序（非产品特性排序），D3→D4 必须做跨维一致性检查 |
| D5 | 消费决策层 | 北极星——先定建议方向再展开研究，竞品价格标注核实时间 |
| D6 | 脚本素材层 | 按 P0-P3 优先级筛选素材，直接喂给脚本阶段 |

**实战教训**：D1-D6 不需要做去AI处理（研究文件的核心价值是数据准确性，不是语言风格），去AI 只在脚本优化阶段做一次。独立搜索任务（如多竞品价格）应并行执行。

### 脚本情绪规则

- **理性骨架 + 情绪血肉** — 纯理性分析缺乏观看动力，B站观众需要情绪起伏
- **情绪词要真实** — "惊艳""气死""离谱""真香"，不怕观点太强
- **行动建议要具体** — "如果你在纠结X，听我的，直接Y"，不只列人群分类
- **互动钩子** — 每段至少一个弹幕互动钩子，开场/中段/结尾都要有

### 「诚实拐点」分散式原则

JJXS 的标志性叙事：**在脚本各处自然融入判断句，而非集中在单一「诚实拐点」段落。**

集中说真话反而让真诚内容变得刻意。判断应散在全片各处——开场建立信任时给一个、中段对比时给一个、结尾购买建议时给一个。让观众感觉「这人一直在说真话」，而不是「等他终于要讲真话了」。
...
| 7 | 写"交付脚本的关键结论" | — | `templates/产品研究模板.md` 第十章，直接喂给脚本阶段 |
...
- [ ] 已知争议和品控问题已覆盖，有自己的判断
- [ ] Raymond 的核心使用场景已分析
- [ ] **"交付脚本的关键结论"已完成**（`templates/产品研究模板.md` 第十章）
- [ ] 所有数据标注了来源类型
...
- **mlx_whisper**：Apple Silicon 本地语音转文字（脚本阶段）

### 外部 Skills（skill-list.md 登记，按需安装）
- **bilibili-analyzer**（管理区）：B站数据分析、视频表现分析
- **vsl-storyboard-writer**（脚本区）：视频分镜脚本写作、镜头规划
...
- **Skills**：xlsx（分镜表）、brainstorming（角度推敲）、executing-plans、humanizer-zh（脚本润色去AI）
- **详细规则**：`workspaces/script-rules.md`

### 工作区三：管理与复盘
- **模型**：DeepSeek V4-Pro，思考模式**默认关闭**（复盘和策略分析时手动开启）
- **角色**：系统管理员 — 资讯处理、选题筛选、四层复盘、系统规则迭代
- **Skills**：tool-builder、find-skills、xlsx（复盘数据表）、karpathy-guidelines（工具开发防坑）
...
- [ ] **工作区二（脚本与分镜）**：已创建，角色=脚本写手，Skills=xlsx/brainstorming/executing-plans/humanizer-zh，思考=开启
- [ ] **工作区三（管理与复盘）**：已创建，角色=管理员，Skills=tool-builder/find-skills/xlsx/karpathy-guidelines，思考=默认关闭
